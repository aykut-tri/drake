...
dir='/home/josebarreiros/'+path[path.find('drake',path.find('drake')+1):]

class VideoCaptureZED:

  def __init__(self, camera_port,camera_device_id=0):
    self.cap = cv.VideoCapture(camera_port)
    self.side=camera_device_id
  ...
  def read(self):
    ...
    if success:
        if self.side==0: #left     
            image=frame[:,:int(self.im_shape[1]/2),:].copy()
...
#    pdb.set_trace()
    ...
    if calibration_settings['camera_model']:
        camera_model=calibration_settings['camera_model']
    if camera_model=="ZED_M":
        camera_port=calibration_settings['camera_port']

    #open video stream and change resolution.
    #Note: if unsupported resolution is used, this does NOT raise an error.
    if camera_model=="ZED_M":
        cap = VideoCaptureZED(camera_port,camera_device_id)
        width=int(cap.im_shape[1]/2)
        height=cap.im_shape[0]
    ...
    while True:
        ...
        if start:
            ...
            if cooldown <= 0:
                savename = os.path.join(directory, camera_name + '_' + str(saved_count) + '.png')
...
def save_camera_intrinsics(camera_matrix, distortion_coefs, camera_name):
    ...
    directory=dir+'/camera_parameters'
    if not os.path.exists(directory):
        os.mkdir(directory)

    out_filename = os.path.join(directory, camera_name + '_intrinsics.dat')
...
def save_frames_two_cams(camera0_name, camera1_name):
    ...
    #Note: if unsupported resolution is used, this does NOT raise an error.
    if camera_model=="ZED_M":
        cap=VideoCaptureZED(camera_port)
        width=int(cap.im_shape[1]/2)
        height=cap.im_shape[0]
        print("width: ",width,"height",height)
    else:
    ...
    while True:
        if camera_model=="ZED_M":
            ret,frame0,frame1=cap.read_stereo()    
            if not ret:
        ...
        if not start:
            ...
            cv.putText(frame1_small, "Make sure both cameras can see the calibration pattern well", (50,50), cv.FONT_HERSHEY_COMPLEX, 1, (0,0,255), 1)
            cv.putText(frame1_small, "Press SPACEBAR to start collection frames", (50,100), cv.FONT_HERSHEY_COMPLEX, 1, (0,0,255), 1)
        ...
        if start:
            ...
            #save the frame when cooldown reaches 0.
            if cooldown <= 0:
                savename = os.path.join(directory, camera0_name + '_' + str(saved_count) + '.png')
                cv.imwrite(savename, frame0)

                savename = os.path.join(directory, camera1_name + '_' + str(saved_count) + '.png')
...
def stereo_calibrate(mtx0, dist0, mtx1, dist1, frames_prefix_c0, frames_prefix_c1):
    ...
    for frame0, frame1 in zip(c0_images, c1_images):
        ...
        if c_ret1 == True and c_ret2 == True:
            ...
            cv.putText(frame1, 'If detected points are poor, press "s" to skip this sample', (25, 25), cv.FONT_HERSHEY_COMPLEX, 1, (0,0,255), 1)
...
def save_extrinsic_calibration_parameters(R0, T0, R1, T1, prefix = ''):
    ...
    camera0_rot_trans_filename = os.path.join(directory, prefix + 'camera0_rot_trans.dat')
    ...
    camera1_rot_trans_filename = os.path.join(directory, prefix + 'camera1_rot_trans.dat')
...
if __name__ == '__main__':
    ...
    #camera0 intrinsics
    images_prefix = os.path.join(dir,'frames', 'camera0*')
    ...
    #camera1 intrinsics
    images_prefix = os.path.join(dir,'frames', 'camera1*')
    ...
    frames_prefix_c0 = os.path.join(dir,'frames_pair', 'camera0*')
    frames_prefix_c1 = os.path.join(dir,'frames_pair', 'camera1*')
