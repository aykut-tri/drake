...
// gripper's main body.
SpatialInertia<double> MakeCompositeGripperInertia_(
...
void get_camera_poses_(std::map<std::string, RigidTransform<double>>* pose_map) {
...
MakeD415CameraModel_(const std::string& renderer_name) {
...
template <typename T>
RlCitoStation<T>::RlCitoStation(double time_step)
...
template <typename T>
void RlCitoStation<T>::SetupClutterClearingStation(
    const std::optional<const math::RigidTransform<double>>& X_WCameraBody,
    IiwaCollisionModel_ collision_model, SchunkCollisionModel_ schunk_model) {
  DRAKE_DEMAND(setup_ == Setup_::kNone);
  setup_ = Setup_::kClutterClearing;

  // Add the bins.
  {
    const std::string sdf_path = FindResourceOrThrow(
        "drake/examples/rl_cito_station/models/bin.sdf");
  ...
  // Add the camera.
  {
    const auto& [color_camera, depth_camera] =
        internal::MakeD415CameraModel_(default_renderer_name_);
...
template <typename T>
void RlCitoStation<T>::SetupManipulationClassStation(
  IiwaCollisionModel_ collision_model,
  SchunkCollisionModel_ schunk_model) {
  DRAKE_DEMAND(setup_ == Setup_::kNone);
  setup_ = Setup_::kManipulationClass;
  ...
  // Add the cupboard.
  {
    ...
    const std::string sdf_path = FindResourceOrThrow(
        "drake/examples/rl_cito_station/models/cupboard.sdf");
  ...
  // Add default cameras.
  {
    ...
    internal::get_camera_poses_(&camera_poses);
    const auto& [color_camera, depth_camera] =
        internal::MakeD415CameraModel_(default_renderer_name_);
...
template <typename T>
void RlCitoStation<T>::SetupPlanarIiwaStation(
  SchunkCollisionModel_ schunk_model) {
  DRAKE_DEMAND(setup_ == Setup_::kNone);
  setup_ = Setup_::kPlanarIiwa;
...
void RlCitoStation<T>::MakeIiwaControllerModel() {
  ...
  const multibody::RigidBody<T>& wsg_equivalent =
      owned_controller_plant_->AddRigidBody(
          ...
          internal::MakeCompositeGripperInertia_(
...
void RlCitoStation<T>::Finalize(
  ...
  switch (setup_) {
    case Setup_::kNone:
    case Setup_::kManipulationClass: {
...
template <typename T>
T RlCitoStation<T>::GetWsgPosition(
    const systems::Context<T>& station_context) const {
  const auto& plant_context =
      this->GetSubsystemContext(*plant_, station_context);
...
template <typename T>
T RlCitoStation<T>::GetWsgVelocity(
    const systems::Context<T>& station_context) const {
  const auto& plant_context =
      this->GetSubsystemContext(*plant_, station_context);
...
template <typename T>
void RlCitoStation<T>::SetWsgPosition(
    const drake::systems::Context<T>& station_context, systems::State<T>* state,
    const T& q) const {
  DRAKE_DEMAND(state != nullptr);
  auto& plant_context = this->GetSubsystemContext(*plant_, station_context);
...
template <typename T>
void RlCitoStation<T>::SetWsgVelocity(
    const drake::systems::Context<T>& station_context, systems::State<T>* state,
    const T& v) const {
  DRAKE_DEMAND(state != nullptr);
  auto& plant_context = this->GetSubsystemContext(*plant_, station_context);
...
template <typename T>
std::vector<std::string> RlCitoStation<T>::get_camera_names() const {
...
template <typename T>
void RlCitoStation<T>::SetWsgGains(const double kp, const double kd) {
...
template <typename T>
void RlCitoStation<T>::RegisterWsgControllerModel(
    const std::string& model_path,
...
template <typename T>
void RlCitoStation<T>::RegisterRgbdSensor(
...
RlCitoStation<T>::GetStaticCameraPosesInWorld() const {
...
// Add default wsg.
template <typename T>
void RlCitoStation<T>::AddDefaultWsg(
    const SchunkCollisionModel_ schunk_model) {
  std::string sdf_path;
  switch (schunk_model) {
    case SchunkCollisionModel_::kBox:
      sdf_path = FindResourceOrThrow(
          "drake/manipulation/models/wsg_50_description/sdf"
          "/schunk_wsg_50_no_tip.sdf");
      break;
    case SchunkCollisionModel_::kBoxPlusFingertipSpheres:
      sdf_path = FindResourceOrThrow(
          "drake/manipulation/models/wsg_50_description/sdf"
